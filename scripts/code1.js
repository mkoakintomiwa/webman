...
(async _=>{
    ...
    if (argv["root-ip"]){
        ...
        User = root.username;
        remoteHomeDir = '/root';
        defaultRemoteDir =  argv._[1] || root.home || '/';
    }else{
        if (argv["root"]){
            ...
            defaultRemoteDir =  argv._[1] || node.home || remoteHomeDir+"/public_html";
        }
    }
    ...
    if (typeof config.find({ Host }) === "undefined"){
        ...
        await fx.shellExec(`ssh-keygen -b 2048 -t rsa -f ${privateKeyPath} -q -N ""`);
        config.append({
            Host,
            User,
            HostName,
